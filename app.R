...
data <- readRDS("./data/air_quality_data_ugm3.rds") %>%
                ...
                box(title = "Sensores de gases e partículas",
                  ...
                  sidebarPanel(
                        selectInput("Poluente", "Escolha o Poluente:",
                                    choices = unique(data$Poluente)),
                       # selectInput("Year", "Escolha o Ano:", choices = unique(data$Year)),
                        selectInput("Cities", "Escolha a Cidade:", choices = unique(data$Cidade),
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlotly(
    plot1 <- data %>%
      ...
      subset(date >= input$start_date2 & date <= input$end_date2) %>%
      arrange(date) %>%
      plot_ly(
      x = ~date,
      y = ~`Concentração (ug/m³)`,
